...
def Encrypt(pt, rkb1, rk1, rkb2, rk2, rkb3, rk3):
	firstTime = bin2hex(encrypt(pt, rkb1, rk1))
	rkb2_rev = rkb2[::-1]
	rk2_rev = rk2[::-1]
	secondTime = bin2hex(encrypt(firstTime, rkb2_rev, rk2_rev))
	thirdTime = bin2hex(encrypt(secondTime, rkb3, rk3))
	return thirdTime

def Decrypt(pt, rkb1, rk1, rkb2, rk2, rkb3, rk3):
	rkb3_rev = rkb3[::-1]
	rk3_rev = rk3[::-1]
	firstTime = bin2hex(encrypt(pt, rkb3_rev, rk3_rev))
	secondTime = bin2hex(encrypt(firstTime, rkb2, rk2))
	rkb1_rev = rkb1[::-1]
	rk1_rev = rk1[::-1]
	thirdTime = bin2hex(encrypt(secondTime, rkb1_rev, rk1_rev))
	return thirdTime
...
def encode_tdes_image_ecb(key1, key2, key3, url):
	...
	# of 2
	if encryptedImg.width % 2 != 0:
		diff = 2 - (encryptedImg.width % 2)
	...
	# 2 pixels to transform them into a new m pixels
	for y in range(0, encryptedImg.height):
		rowPixels = ""
		for x in range(0, encryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Transform the 2 pixels using T-DES cipher
				newRowPixels = Encrypt(rowPixels, rkb1, rk1, rkb2, rk2, rkb3, rk3)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					encryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
				rowPixels = ""
			rowPixels += DecimalToHex(encryptedImg.getpixel((x,y)))
...
def decode_tdes_image_ecb(key1, key2, key3, url):
	...
	# Iterate over each row of the image height taking at each step
	# m pixels to transform them into a new 2 pixels
	for y in range(0, decryptedImg.height):
		rowPixels = ""
		for x in range(0, decryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Transform the 2 pixels using the T-DES cipher
				newRowPixels = Decrypt(rowPixels, rkb1, rk1, rkb2, rk2, rkb3, rk3)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					decryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
				rowPixels = ""
			
			rowPixels += DecimalToHex(decryptedImg.getpixel((x,y)))
...
def encode_tdes_image_cbc(key1, key2, key3, iv, url):
	...
	# Iterate over each row of the image height taking at each step
	# 2 pixels to transform them into a new 2 pixels
	Y = [iv]
	for y in range(0, encryptedImg.height):
		rowPixels = ""
		for x in range(0, encryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Make a XOR
				rowPixels = bin2hex(xor(hex2bin(rowPixels), hex2bin(Y[-1])))
				# Transform the 2 pixels using T-DES cipher
				newRowPixels = Encrypt(rowPixels, rkb1, rk1, rkb2, rk2, rkb3, rk3)
				Y.append(newRowPixels)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					encryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
				rowPixels = ""
	...
	for y in range(0, decryptedImg.height):
		...
		for x in range(0, decryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Transform the 2 pixels using the T-DES cipher
				newRowPixels = Decrypt(rowPixels, rkb1, rk1, rkb2, rk2, rkb3, rk3)
				newRowPixels = bin2hex(xor(hex2bin(newRowPixels), hex2bin(Y[-1])))
				Y.append(rowPixels)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					decryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
...
def encode_tdes_image_ofb(key1, key2, key3, iv, url):
	...
	for y in range(0, encryptedImg.height):
		...
		for x in range(0, encryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Get the next Z_i using T-DES cipher
				Z.append(Encrypt(Z[-1], rkb1, rk1, rkb2, rk2, rkb3, rk3))
				# Make a XOR
				newRowPixels = bin2hex(xor(hex2bin(rowPixels), hex2bin(Z[-1])))
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					encryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
				rowPixels = ""
...
def encode_tdes_image_cfb(key1, key2, key3, iv, url):
	...
	for y in range(0, encryptedImg.height):
		...
		for x in range(0, encryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Get the z_i using T-DES cipher
				z_i = Encrypt(Y[-1], rkb1, rk1, rkb2, rk2, rkb3, rk3)
				# Make a XOR
				newRowPixels = bin2hex(xor(hex2bin(rowPixels), hex2bin(z_i)))
				# Save the ciphertext for z_i+1
				Y.append(newRowPixels)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					encryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
	...
	for y in range(0, decryptedImg.height):
		...
		for x in range(0, decryptedImg.width):
			if x % 2 == 0 and x > 0:
				...
				# Make a XOR
				newRowPixels = bin2hex(xor(hex2bin(rowPixels), hex2bin(z_i)))
				# Save the current ciphertext to get z_i+1
				Y.append(rowPixels)
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					decryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
...
def encode_tdes_image_ctr(key1, key2, key3, ctr, url):
	...
	for y in range(0, encryptedImg.height):
		...
		for x in range(0, encryptedImg.width):
			if x % 2 == 0 and x > 0:
				# Get T_i
				T_i = bin2hex(dec2bin(((int(ctr, base=16) + (count - 1)) % (2**64))))
				count+=1
				# Encrypt T_i using T-DES cipher
				T_i = Encrypt(T_i, rkb1, rk1, rkb2, rk2, rkb3, rk3)
				# Make a XOR
				newRowPixels = bin2hex(xor(hex2bin(rowPixels), hex2bin(T_i)))
				newRowPixels = HexToDecimal(newRowPixels)
				for i in range(x - 2, x):
					encryptedImg.putpixel((i,y), newRowPixels[i - (x-2)])
				rowPixels = ""
